...
def create_case_string(row, sep=","):
  ...
  if row['includes_primary']:
    primary_strings.append(f"DIAG_4_CONCAT_PRIMARY RLIKE {convert_list_to_regex_string(row['includes_primary'], sep)}")
  if row['excludes_primary']:
    primary_strings.append(f"DIAG_4_CONCAT_PRIMARY NOT RLIKE {convert_list_to_regex_string(row['excludes_primary'], sep)}")
  if row['excludes_secondary']:
    primary_strings.append(f"DIAG_4_CONCAT_SECONDARY NOT RLIKE {convert_list_to_regex_string(row['excludes_secondary'], sep)}")
  primary_string = ' AND '.join(primary_strings) 
    
  secondary_strings = []
  if row['includes_secondary']:
    secondary_strings.append(f"DIAG_4_CONCAT_SECONDARY RLIKE {convert_list_to_regex_string(row['includes_secondary'], sep)}")
  if row['excludes_secondary']:
    secondary_strings.append(f"DIAG_4_CONCAT_SECONDARY NOT RLIKE {convert_list_to_regex_string(row['excludes_secondary'], sep)}")
  if row['excludes_primary']:
    secondary_strings.append(f"DIAG_4_CONCAT_PRIMARY NOT RLIKE {convert_list_to_regex_string(row['excludes_primary'], sep)}")
...
def create_categorical_definition_string(case_statements):
  return f"CASE\n      {case_statements.str.cat(sep='      ')}      ELSE 'Other'\n    END AS TYPE_OF_ADMISSION"
...
SELECT
  *,
  -- ROW_NUMBER() OVER (PARTITION BY PERSON_ID_DEID ORDER BY CASE WHEN TYPE_OF_ADMISSION NOT RLIKE 'Exclude' THEN 1 WHEN TYPE_OF_ADMISSION NOT RLIKE 'Other' THEN 2 ELSE 3 END, ADMIDATE ASC) AS ADMISSION_IDX,
  CASE WHEN TYPE_OF_ADMISSION RLIKE 'Other' THEN 'Non-COVID' WHEN TYPE_OF_ADMISSION RLIKE 'Exclude' THEN 'COVID Excluded' WHEN TYPE_OF_ADMISSION RLIKE 'No Type' THEN 'COVID Missed' ELSE 'COVID' END AS META_TYPE
FROM (
  SELECT
    *,
