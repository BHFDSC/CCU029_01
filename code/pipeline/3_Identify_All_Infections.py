...
admissions_col_names = spark.sql(f"SELECT * FROM .{admissions_table_name}").schema.names
tests_col_names = spark.sql(f"SELECT * FROM .{tests_table_name}").schema.names
...
FROM (
  ...
  FROM (
    SELECT
      ...
      {cols_to_include}
    FROM .{admissions_table_name} a
    -- FROM (SELECT * FROM (SELECT *, ROW_NUMBER() OVER (PARTITION BY PERSON_ID_DEID ORDER BY ADMIDATE ASC) AS ADMISSION_IDX FROM .{admissions_table_name} WHERE META_TYPE == 'COVID') WHERE ADMISSION_IDX == 1) a
    FULL OUTER JOIN .{tests_table_name} b
    ON 
...
if test:
  display(spark.sql(f"SELECT SUM(CASE WHEN COVID_ICU == 1 THEN 1 ELSE 0 END) AS NUM_ICU, SUM(CASE WHEN COVID_DAY_CASE == 1 THEN 1 ELSE 0 END) AS NUM_DAY_CASE, SUM(CASE WHEN COVID_STILL_IN_HOSPITAL == 1 THEN 1 ELSE 0 END) AS NUM_STILL_IN_HOSPITAL FROM .{output_table_name}"))

# COMMAND ----------

if test:
  display(spark.sql(f"SELECT COUNT(*), COUNT(DISTINCT PERSON_ID_DEID) FROM .{output_table_name}"))

# COMMAND ----------

if test:
  display(spark.sql(f"SELECT INFECTION_SOURCE, COUNT(*) FROM .{output_table_name} GROUP BY INFECTION_SOURCE"))

# COMMAND ----------

display(spark.sql(f"SELECT COUNT(*) AS COVID_ADMISSIONS_IN_COHORT FROM .{output_table_name} WHERE COVID_ADMISSION_IN_WINDOW == 1"))

# COMMAND ----------

if test:
  display(spark.sql(f"SELECT COUNT(*) FROM .{output_table_name} WHERE ADMISSION_IN_WINDOW == 1"))

# COMMAND ----------

if test:
  display(spark.sql(f"SELECT META_TYPE, COUNT(*) FROM .{output_table_name} GROUP BY META_TYPE"))

# COMMAND ----------

if test:
  display(spark.sql(f"SELECT TYPE_OF_ADMISSION, META_TYPE, COUNT(*) FROM .{output_table_name} WHERE TYPE_OF_ADMISSION NOT RLIKE 'Exclude' GROUP BY TYPE_OF_ADMISSION, META_TYPE ORDER BY META_TYPE, TYPE_OF_ADMISSION"))
...
# MAGIC   FROM .ccu029_01_wip_cohort
