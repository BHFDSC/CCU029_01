...
try:
  input_table_name = f".{cohort_w_lookback_output_table_name}"
...
table = spark.sql(f"SELECT * FROM .{cohort_all_weights_table_name}")
...
table = spark.sql(f"SELECT * FROM .{cohort_all_heights_table_name}")
...
table = spark.sql(f"SELECT * FROM .{cohort_best_weight_and_height_table_name}")
...
# MAGIC FROM global_temp.ccu029_01_cohort_all_weights_table_name a
# MAGIC -- ANTI JOIN .ccu029_01_wip_hosp_cohort_best_weight_and_height b
