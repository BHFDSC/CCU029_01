...
table = spark.sql(f"SELECT * FROM .{hes_apc_demographics_table_name}")
...
table = spark.sql(f"SELECT * FROM .{filtered_spells_table_name}")
print(f"`{filtered_spells_table_name}` has {table.count()} rows and {len(table.columns)} columns.")

# COMMAND ----------

if test:
  display(spark.sql(f"SELECT LENGTH_OF_STAY, * FROM .{filtered_spells_table_name} ORDER BY LENGTH_OF_STAY DESC"))
