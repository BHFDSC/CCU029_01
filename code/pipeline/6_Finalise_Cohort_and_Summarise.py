...
try:
  input_table_name = f".{cohort_w_vaccinations_output_table_name}"
...
table = spark.sql(f"SELECT * FROM .{output_table_name}")
...
FROM .{output_table_name} WHERE AGE BETWEEN 5 AND 11.99999"""))
...
if test:
  display(spark.sql(f"SELECT SUM(CASE WHEN INFECTION_VARIANT_PERIOD == 'Omicron' AND AGE_CAT == '< 1' THEN 1 ELSE 0 END) AS N_INFANT_OMICRON_OUT_OF_TOTAL_ADMISSIONS, ROUND(100 * SUM(CASE WHEN INFECTION_VARIANT_PERIOD == 'Omicron' AND AGE_CAT == '< 1' THEN 1 ELSE 0 END) / COUNT(*), 1) AS PERC_INFANT_OMICRON_OUT_OF_TOTAL_ADMISSIONS FROM .{output_table_name} WHERE COVID_ADMISSION_IN_WINDOW == 1"))

# COMMAND ----------

if test:
  display(spark.sql(f"SELECT CASE WHEN COVID_ADMISSION_IN_WINDOW == 1 THEN 'COVID Admission' ELSE 'Positive Test Only' END AS GROUP, ROUND(100 * SUM(CASE WHEN WEIGHT IS NOT NULL THEN 1 ELSE 0 END) / COUNT(*), 1) AS PERC_WEIGHT_PRESENT, ROUND(100 * SUM(CASE WHEN HEIGHT IS NOT NULL THEN 1 ELSE 0 END) / COUNT(*), 1) AS PERC_HEIGHT_PRESENT, ROUND(100 * SUM(CASE WHEN BMI IS NOT NULL THEN 1 ELSE 0 END) / COUNT(*), 1) AS PERC_BMI_PRESENT FROM .{output_table_name} GROUP BY CASE WHEN COVID_ADMISSION_IN_WINDOW == 1 THEN 'COVID Admission' ELSE 'Positive Test Only' END"))

# COMMAND ----------

if test:
  display(spark.sql(f"SELECT ROUND(100 * SUM(CASE WHEN POSITIVE_TEST_SPECIMEN_DATE BETWEEN DATE_ADD(ADMIDATE, -42) AND DATE_ADD(ADMIDATE, -1) THEN 1 ELSE 0 END) / COUNT(*), 1) AS PERC_PIMS_WITH_TEST_42_TO_1, ROUND(100 * SUM(CASE WHEN POSITIVE_TEST_SPECIMEN_DATE BETWEEN DATE_ADD(ADMIDATE, -42) AND ADMIDATE THEN 1 ELSE 0 END) / COUNT(*), 1) AS PERC_PIMS_WITH_TEST_42_TO_0 FROM .{output_table_name} WHERE TYPE_OF_ADMISSION == 'PIMS'"))

# COMMAND ----------

if test:
  display(spark.sql(f"SELECT SUM(CASE WHEN COVID_ICU == 1 OR TYPE_OF_ADMISSION == 'PIMS' THEN 1 ELSE 0 END) FROM .{output_table_name}"))

# COMMAND ----------

if test:
  display(spark.sql(f"SELECT TYPE_OF_ADMISSION, COUNT(*), SUM(POSITIVE_COVID_TEST_INCLUSION), (SUM(POSITIVE_COVID_TEST_INCLUSION) / COUNT(*)) * 100 FROM .{output_table_name} GROUP BY TYPE_OF_ADMISSION"))

# COMMAND ----------

if test:
  display(spark.sql(f"""SELECT CASE WHEN TYPE_OF_ADMISSION RLIKE 'Type A' THEN 'Type A' WHEN TYPE_OF_ADMISSION RLIKE 'Type B' THEN 'Type B' ELSE TYPE_OF_ADMISSION END, 100 * SUM(CASE WHEN POSITIVE_COVID_TEST_INCLUSION == 1 THEN 1 ELSE 0 END) / COUNT(*) FROM .{output_table_name} GROUP BY CASE WHEN TYPE_OF_ADMISSION RLIKE 'Type A' THEN 'Type A' WHEN TYPE_OF_ADMISSION RLIKE 'Type B' THEN 'Type B' ELSE TYPE_OF_ADMISSION END"""))
...
ORDER BY N_OCCURRENCES DESC""")
  ...
  table = spark.sql(f"SELECT * FROM .{table_name} ORDER BY N_OCCURRENCES DESC")
...
if test:
  display(spark.sql(f"SELECT COVID_DAY_CASE, COUNT(*) FROM .{output_table_name} GROUP BY COVID_DAY_CASE"))
...
INNER JOIN .curr302_patient_skinny_record b
