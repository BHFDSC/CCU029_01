...
try:
  input_table_name = f".{first_infections_output_table_name}"
...
table = spark.sql(f"SELECT * FROM .{cohort_hes_apc_lookback_table_name}")
...
table = spark.sql(f"SELECT * FROM .{cohort_hes_op_lookback_table_name}")
...
table = spark.sql(f"SELECT * FROM .{cohort_gdppr_lookback_table_name}")
...
if test:
  display(spark.sql(f"SELECT COUNT(*) FROM .{output_table_name} WHERE APA_DIAG NOT RLIKE 'U071|U072|U073|U074'"))

# COMMAND ----------

if test:
  display(spark.sql(f"SELECT APA_DIAG, APA_HES_APC_DIAG, APA_HES_OP_DIAG, APA_GDPPR_DIAG, APA_COUNT, APA_HES_APC_COUNT, APA_HES_OP_COUNT, APA_GDPPR_COUNT, APA_HES_APC_DATES, * FROM .{output_table_name}"))
...
if test:
  ...
  n_ad_look = spark.sql(f"SELECT COUNT(*) FROM .{output_table_name}").first()[0]
  print(n, n_ad_look)
  assert n == n_ad_look
