...
try:
  input_table_name = f".{admissions_w_icu_output_table_name}"
...
display(spark.sql(f"SELECT TYPE_OF_ADMISSION, META_TYPE, COUNT(*) FROM .{output_table_name} GROUP BY TYPE_OF_ADMISSION, META_TYPE ORDER BY META_TYPE, TYPE_OF_ADMISSION"))

# COMMAND ----------

display(spark.sql(f"SELECT TYPE_OF_ADMISSION, META_TYPE, COUNT(*) FROM .{output_table_name} WHERE (PIMS_REQUIREMENT == 1 OR COVID_PRIMARY_HOSPITALISATION == 1 OR COVID_DEFINITE_SECONDARY_HOSPITALISATION == 1 OR POSITIVE_COVID_TEST_INCLUSION == 1) GROUP BY TYPE_OF_ADMISSION, META_TYPE ORDER BY META_TYPE, TYPE_OF_ADMISSION"))

# COMMAND ----------

display(spark.sql(f"SELECT 100 * SUM(CASE WHEN META_TYPE == 'COVID Missed' THEN 1 ELSE 0 END) / COUNT(*) AS PERCENT_MISSED, 100 * SUM(CASE WHEN META_TYPE == 'COVID Excluded' THEN 1 ELSE 0 END) / COUNT(*) AS PERCENT_EXCLUDED FROM .{output_table_name} WHERE (PIMS_REQUIREMENT == 1 OR COVID_PRIMARY_HOSPITALISATION == 1 OR COVID_DEFINITE_SECONDARY_HOSPITALISATION == 1 OR POSITIVE_COVID_TEST_INCLUSION == 1)"))
...
INNER JOIN .{output_table_name} b
...
if test:
  display(spark.sql(f"SELECT LENGTH_OF_STAY, COUNT(*) FROM .{output_table_name} WHERE TYPE_OF_ADMISSION == 'Nosocomial' GROUP BY LENGTH_OF_STAY ORDER BY LENGTH_OF_STAY"))
