...
display(spark.sql(f"SELECT 100 * SUM(CASE WHEN META_TYPE != 'COVID' THEN 1 ELSE 0 END) / COUNT(*) AS PERCENT_MISSED FROM dars_nic_391419_j3w9t_collab.{output_table_name} WHERE (PIMS_REQUIREMENT == 1 OR COVID_PRIMARY_HOSPITALISATION == 1 OR COVID_DEFINITE_SECONDARY_HOSPITALISATION == 1 OR POSITIVE_COVID_TEST_INCLUSION == 1)"))
